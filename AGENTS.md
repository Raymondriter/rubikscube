...
**Cube skins shipped free** (`RUBIKSCUBE-6`, Phase 5's last open scope item): `CUBE_SKINS` had been built and tested since Phase 5 but was wired to nothing - no picker, no persisted choice, so it shipped as dead code no user could reach. It fell in the gap between Phase 5 (which scoped the skins) and Phase 7 (which moved *selling* them to a separate epic). Now a radio picker in Settings, free, with no entitlement: `skinId` in `ProgressSettings` (`PROGRESS_VERSION` 7 -> 8 so existing snapshots actually receive it), `setSkinId` on the store, and `setCubeSkin` pushed into the renderer from `CubeCanvas` the same way `setColorblindStickers` is. Selling skins remains open in `RUBIKSCUBE-45`; nothing here presumes it.

The catalog moved to `src/engine/render/skins.ts`, deliberately free of any `three` import: `materials.ts` pulls THREE, which code-splits into its own ~560KB chunk, and both `progress.ts` (main bundle) and the Settings page need the ids/names. Verified the split holds - main bundle stayed at 306KB after the change. Same class of coupling as the `TrainerHomePage` lesson below: check what an import actually drags in.
